...
KT.test('授業の問題＋：にが手ノートと「なぜまちがい？」（まず手帳の中でさがす）', async function(){
  ...
  /* 手帳の中の表で説明できるものは、AIを使わない（基準値の問題を自分で作って、わざとまちがえておく）
     ほかのテストが作った問題に左右されないように、ここで作ったものでためす。 */
  A.qzAddQs([{ qt:'mc', q:'白血球数（WBC）の基準値はどれか。',
    c:['3,300〜8,600 /μL', '1万〜2万 /μL', '100〜200 /μL', '30〜50 /μL'], a:[0], exp:'', lv:1 }],
    A.qzSubs()[0].id, '', 'くらしの手帳の基準値表');
  var q = A.qzQs('').filter(function(x){ return /白血球数/.test(x.q); })[0];
  ok(q, '基準値の問題を作れた');
  A.qzLogAnswer(q.id, false);
  A.commit();
  ok(A.qz2WeakList('all').some(function(o){ return o.q.id === q.id; }), 'にが手ノートに入る');
  var before = KT.aiCalls.length;
  var why = A.qz2Why(q);
...
KT.test('授業の問題＋：計算の○×を自分で直す・といた記録をまとめて消す', async function(){
  var A = KT.frames().A, B = KT.frames().B;
  KT.freshWrites([A, B]);
  var sub = A.qzSubAdd('看護の計算');
  var q = A.qzPush({ id:'qz_flip1', mt:Date.now(), mod:'quiz', type:'q', sub:sub.id, mat:'', qt:'calc',
    q:'600mLの輸液を7時間で落とすとき、1分間の滴下数は？', at:'29', un:'滴/分', tol:5,
    how:'600×20÷420＝28.6', exp:'', src:'テスト' });
  A.commit();
  /* わざと遠い数を書いて、まちがいにする */
  A.qzState.sub = sub.id;
  A.studyTool = 'qz-drill';
  var day0 = A.qzDayCount(A.today()).n;
  ok(A.qzStart(sub.id, 'all', 1), '1問はじめる');
  A.render();
  A.document.getElementById('qz_ans').value = '50';
  A.qzState.inp.qz_ans = '50';
  click(A, '[data-act="qz-check"]');
  await KT.until(function(){ return A.qzState.run && A.qzState.run.shown; }, 3000, '答えあわせ');
  eq(A.qzState.run.res, 0, 'まちがいになる');
  eq(A.toNum(A.qzLogOf(q.id).miss), 1, '記録もまちがい1回');
  /* 「やっぱり正かい」でひっくり返す */
  ok(/やっぱり/.test(appText(A)), '直すボタンが出る');
  click(A, '[data-act="qz-flip"]');
  await KT.until(function(){ return A.qzState.run.res === 1; }, 3000, 'ひっくり返る');
  var l = A.qzLogOf(q.id);
  eq(A.toNum(l.n), 1, 'といた回数は1回のまま（二重にかぞえない）');
  eq(A.toNum(l.ok), 1, '正解1回');
  eq(A.toNum(l.miss), 0, 'まちがいは0回にもどる');
  eq(A.qzDayCount(A.today()).n, day0 + 1, '今日といた数も1回ぶんだけ（二重にかぞえない）');
  ok(l.due > A.today(), '次に出す日も、正解として入れ直される');
  /* といた記録を、まとめて消す */
  A.qzState.run = null;
  A.studyTool = 'qz-lib';
  A.qzState.libTab = 'q';
  A.render();
  click(A, '[data-act="qz-log-reset"][data-v="ask"]');
  ok(/記録（正解・まちがい/.test(appText(A)), '消す前に、たしかめてくれる');
  click(A, '[data-act="qz-log-reset"][data-v="go"]');
  await KT.until(function(){ return !A.qzLogOf(q.id); }, 3000, '記録が消える');
  ok(A.qzItem(q.id), '問題はのこっている');
  await KT.settle([A, B]);
  ok(B.qzItem(q.id) && !B.qzLogOf(q.id), 'もう1台でも、問題はのこって記録だけ消える');
});
})();