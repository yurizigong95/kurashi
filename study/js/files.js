...
function linkOnly(text){
  ...
  return !!t && linkList(t).length > 0 && !t.replace(/https?:\/\/\S+/g, '').replace(/[\s、,]+/g, '');
}
...
async function linkFetch(u){
  ...
  try{
    ...
    if(/pdf/.test(type) || /\.pdf(\?|#|$)/i.test(u)) return { kind:'pdf', blob:await res.blob() };
...
async function loadLinkDirect(u){
  ...
  var ext = { pdf:'.pdf', photo:'.jpg', audio:'.mp3', video:'.mp4' }[got.kind] || '';
  var fname = /\.[a-z0-9]{2,4}$/i.test(base) ? base : base + ext, file = null;
  try{ file = new File([got.blob], fname, { type:got.blob.type || '' }); }
  catch(e){ file = got.blob; file.name = fname; }
